...
# In theory if you understand your amplicon sequencing setup, this is 
...
# the presence and orientation of these primers in the data.
...
FWD.orients <- allOrients(forprim)
REV.orients <- allOrients(revprim)
FWD.orients
REV.orients
...
library(ShortRead)
library(Biostrings)
packageVersion("dada2") # [1] ‘1.18.0’
packageVersion("ShortRead") # [1] ‘1.46.0’
packageVersion("Biostrings") # [1] ‘2.56.0’
...
sample.names <- paste0(sample.names0,"_",sampdate)
...
pdf(paste0("Figures/sequences/",sampdate,"/qscores_",region,"/F_control.pdf"), height=3, width=3)
dada2::plotQualityProfile(fnFs[c(81)]) # forward reads
dev.off()
...
pdf(paste0("Figures/sequences/",sampdate,"/qscores_",region,"/R_control.pdf"), height=3, width=3)
dada2::plotQualityProfile(fnRs[c(81)]) # forward reads
dev.off()
...
# Test: not working and here is the error: invalid class “SRFilterResult” object: superclass "Mnumeric" not defined in the environment of the object's class
# Count the number of times the primers appear in the forward and reverse 
# read, while considering all possible primer orientations. Identifying and 
# counting the primers on one set of paired end FASTQ files is sufficient, 
# assuming all the files were created using the same library preparation, 
# so we’ll just process the first sample.

# Place filtered files in filtered/ subdirectory
filtFs <- file.path(path, "filtered0", paste0(sample.names, "_F_filt.fastq.gz"))
filtRs <- file.path(path, "filtered0", paste0(sample.names, "_R_filt.fastq.gz"))
names(filtFs) <- sample.names
names(filtRs) <- sample.names

out0 <- dada2::filterAndTrim(fnFs[1], filtFs[1], fnRs[1], filtRs[1], maxN = 0, multithread = TRUE) 
head(out)
...
## 2484 sequence variants were inferred from 29305 input unique sequences.
