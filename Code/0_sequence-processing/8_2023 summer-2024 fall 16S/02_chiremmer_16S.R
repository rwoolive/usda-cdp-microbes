...
seqtables <- "rocky_dada_16S/output/01_errors-output/rocky_dada_16S/rocky_dada_16S_seqtab.rds" #change to match your file structure
output <- "rocky_dada_16S/output" # where we will dump the results//diagnostic
name <- "rocky_dada_16S" # Run name. A directory for the values will be created
#trim_length <- as.integer(c(350,575)) #trim reads shorter or longer
...
write.csv(paste0('# ',
num.chimera.removed,
" chimera were found and removed\n",
'# These represent ',
perc.num.chimera.removed,
'% of total ASVs and ',
perc.reads.chimera.removed,
'% of total reads\n\n'), paste0(output,"/02_chimera_metadata.csv"))
...
write.csv(table(nchar(getSequences(seqtab.raw))),
paste0(output,"/02_chimera_length_distribution_of_ASVs.csv"))
...
#seqtab <- collapseNoMismatch(seqtab,  minOverlap = 50, verbose=T)
