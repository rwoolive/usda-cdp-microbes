...
seqtab.nochim <- "Taxon_rocky_ITS/4seqtab_ITS.csv" #change to your file structure
output_ITS <- "Taxon_rocky_ITS/output/"
name <- "Taxon_rocky_ITS"
tax_db <- "Taxon_rocky_ITS/sh_general_release_dynamic_27.10.2022.fasta" #change to your tax database (Silva, UNITE, MaarjAM*)
method <- "dada"
threshold <- as.integer(50)

dir.create(file.path(output_ITS), showWarnings = FALSE)
...
# Write to disk
saveRDS(taxa_0, paste0(output_ITS, name, "_tax_assignation_1.rds"))

cat('\n')
cat(paste0('# The obtained taxonomy file can be found in "', paste0(output_ITS, name, "_tax_assignation_1.rds"), '"\n'))
cat('\n# All done!\n\n')
