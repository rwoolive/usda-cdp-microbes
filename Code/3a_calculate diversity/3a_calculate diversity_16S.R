...
dat$Cover <- soildat$Cover
dat$Cropping.system <- soildat$Cropping.system
# ordering <- order(dat$year, dat$season, dat$plot)
# dat <- dat[ordering,]
write.csv(dat, "Processed-data/sequences/sample-metadata-16S.csv")
...
abunds3 <- abunds3[-which(rownames(abunds3) %in% c("203_Summer_2024", "114_Fall_2024")),]
...
########## Simpson's:
### (1 - sum p_i^2)
# Simpson's Index (D) measures the probability that two individuals randomly selected from a sample will belong to the same species (or some category other than species).
...
########## Inverse Simpson's:
### (1/sum p_i^2)
# So inverse Simpson (1/D) starts with 1, and the higher the value the greater the diversity.
