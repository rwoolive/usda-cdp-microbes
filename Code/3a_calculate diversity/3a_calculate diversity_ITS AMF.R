...
#taxa2 <- taxa[which(taxa$Primary_lifestyle %in% c("arbuscular_mycorrhizal")),]
...
# # Get top families
# tser_p <- agglomerateByRank(tser, rank="Family")
# top_taxa <- getTop(tser_p, top = 6, assay.type = "relabundance")
# 
# 
# # Renaming the "Family" rank to keep only top taxa and the rest to "Other"
# Family_renamed <- lapply(rowData(tser)$Family, function(x){if (x %in% top_taxa) {x} else {"Other"}})
# rowData(tser)$Family <- as.character(Family_renamed)
# 
# plot by families
p <- plotAbundance(tser, assay.type="relabundance", group="Family", order.row.by="abund",
                   add_legend=T, as.relative=T, col.var = "Time") 
q <- patchwork::wrap_plots(p, ncol = 1, heights = c(0.95, 0.05))
ggpubr::ggexport(q, filename = paste0("Figures/relative abundance/", subset_name, ".png"), height=1700, width=3000, res = 300)

# plot by phylum, average by time
tser_f <- agglomerateByVariable(tser, by="cols", group="Time", update.tree=T)
p <- plotAbundance(tser_f, assay.type="relabundance", group="Family", order.row.by="abund",
...
min(rowSums(abunds5[,1:length(sampsize)])) # number of reads per asv
