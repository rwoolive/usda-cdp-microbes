...
taxa3 <- taxa[which(taxa$Primary_lifestyle %in% c("plant_pathogen")),]
#taxa3 <- taxa[which(taxa$Phylum %in% c("p__Glomeromycota")),]
taxa3$asv_number <- as.numeric(1:dim(taxa3)[1])
taxa <- taxa3
...
abundances <- abundances[which(abundances$asv_number %in% taxa$asv_number),]
...
# dat <- dat[ordering,]
write.csv(dat, paste0("Processed-data/sequences/sample-metadata-", region,"_", subset_name, ".csv"))
...
write.csv(abundances, paste0("Raw-data/sequence/",sampdate,"/",name,"/seqtab/5seqtab_ordered","_", subset_name,".csv"))
...
TAX$Family <- gsub(pattern="f__", replacement="", TAX$Family )
TAX$Class <- gsub(pattern="c__", replacement="", TAX$Class )
rownames(TAX) <- paste0("sp", row.names(taxa))
colnames(ASV) <- paste0("sp", row.names(taxa))
...
# Get top family
tser_p <- agglomerateByRank(tser, rank="Class")
top_taxa <- getTop(tser_p, top = 10, assay.type = "relabundance")


# Renaming the "Genus" rank to keep only top taxa and the rest to "Other"
phylum_renamed <- lapply(rowData(tser)$Class, function(x){if (x %in% top_taxa) {x} else {"Other"}})
rowData(tser)$Class <- as.character(phylum_renamed)
# for(i in 1:length(rowData(tser)$Genus)){
#   if(rowData(tser)$Genus[i] %in% top_taxa) {rowData(tser)$Genus[i] <- rowData(tser)$Genus[i]}
#   else{rowData(tser)$Genus[i] <- "Other"}
# }


# plot by phylum
p <- plotAbundance(tser, assay.type="relabundance", group="Class", order.row.by="abund",
                   add_legend=T, as.relative=T, col.var = "Time") + theme(legend.key.size = unit(0.4, 'cm'))
q <- patchwork::wrap_plots(p, ncol = 1, heights = c(0.95, 0.05)) 
ggpubr::ggexport(q, filename = paste0("Figures/relative abundance/", subset_name,".png"), height=1800, width=3000, res = 300)

# plot by phylum, average by time
tser_f <- agglomerateByVariable(tser, by="cols", group="Time", update.tree=T)
p <- plotAbundance(tser_f, assay.type="relabundance", group="Class", order.row.by="abund",
                   add_legend=T, as.relative=T) +
  theme(axis.text.x = element_text(angle = 270,  hjust=0), legend.key.size = unit(0.5, 'cm'))
ggpubr::ggexport(p, filename = paste0("Figures/relative abundance/", subset_name,"_averaged.png"), height=1300, width=2000, res = 300)
...
#          filename = paste0("Figures/sequences/", name, "_asv abundance curve","_", subset_name,".png"))
# 
...
write.csv(data.frame(sample=names(remove), seqs=remove), paste0("Raw-data/sequence/",sampdate,"/",name,"/seqtab/6removed","_", subset_name,".csv"))
...
min(rowSums(abunds5[,1:length(sampsize)])) # do some ASVs now have <10 reads?
...
write.csv(taxabund, paste0("Raw-data/sequence/",sampdate,"/",name,"/seqtab/6seqtab","_", subset_name, ".csv"))
...
divmat[1:10,1:10] 
divmat_pa[1:10,1:10] 
write.csv(divmat_pa,  paste0("Raw-data/sequence/",sampdate,"/divmat_pa_", region,"_", subset_name, ".csv"))
...
write.csv(divdat, paste0("Processed-data/sequences/", region,"_", subset_name, "-diversity.csv"))
