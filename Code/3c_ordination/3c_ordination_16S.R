...
with(dat_plot, legend("topleft", legend=levels(Season.Year), bty="n", col="black", pch=21, pt.cex=1.5, pt.bg=alpha(ssnyrcols,0.8), title="Timepoint", title.adj = 0.1, title.font = 2)) # [-which(divdat$new.name %in% rem),]
with(dat_plot, legend("bottomleft", legend=levels(Cropping.system), bty="n", col="black", pch=21:24, pt.cex=1.5, pt.bg="white", title="Cropping system", title.adj = 0.1, title.font = 2)) # [-which(divdat$new.name %in% rem),]
