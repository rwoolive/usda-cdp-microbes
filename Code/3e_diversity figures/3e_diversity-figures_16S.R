...
##################### Import and merge diversity data #####################
dat_16S <- read.csv("Processed-data/sequences/16S-diversity.csv")
dat_ITS <- read.csv("Processed-data/sequences/ITS-diversity.csv")
dat_ITS_AMF <- read.csv("Processed-data/sequences/ITS_AMF-diversity.csv")
dat_ITS_plantpath <- read.csv("Processed-data/sequences/ITS_plant pathogen-diversity.csv")
dat_ITS_sap <- read.csv("Processed-data/sequences/ITS_saprotroph-diversity.csv")

dat_meta <- dat_16S %>% 
  dplyr::select(Replicate, Year, Season, Cropping.system, Cover, Season.Year)
dat_16S <- dat_16S %>% 
  dplyr::select(richness, shannon.div, simpson.div, invsimpson.div, evenness) %>% 
  rename_with(~ paste0("bac_", .x))
dat_relabund <- dat_ITS %>% 
  dplyr::select(amf_relabund, plantpath_relabund, sap_relabund)
dat_ITS <- dat_ITS %>% 
  dplyr::select(richness, shannon.div, simpson.div, invsimpson.div, evenness)%>% 
  rename_with(~ paste0("fun_", .x))
dat_ITS_AMF <- dat_ITS_AMF %>% 
  dplyr::select(richness, shannon.div, simpson.div, invsimpson.div, evenness)%>% 
  rename_with(~ paste0("amf_", .x))
dat_ITS_plantpath <- dat_ITS_plantpath %>% 
  dplyr::select(richness, shannon.div, simpson.div, invsimpson.div, evenness)%>% 
  rename_with(~ paste0("plantpath_", .x))
dat_ITS_sap <- dat_ITS_sap %>% 
  dplyr::select(richness, shannon.div, simpson.div, invsimpson.div, evenness)%>% 
  rename_with(~ paste0("sap_", .x))

dat <- cbind(dat_meta, dat_16S, dat_ITS, dat_ITS_AMF, dat_ITS_plantpath, dat_ITS_sap, dat_relabund)

write.csv(dat, "Processed-data/WTREC-CDP_microbial diversity dat.csv")
