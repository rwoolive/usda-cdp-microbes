...
for(i in 1:length(taxabund$Genus)){
  if(is.na(taxabund$Genus[i])==FALSE){
    taxabund$Genus[i]<- paste0("g__",taxabund$Genus[i])
  }
}
# import sequence metadata
seqdat <- read.csv(paste0("Processed-data/sequences/ITS-diversity.csv"))

### make a phyloseq object with:
# asv_table: samples as columns, asvs as rows
asv_table <- taxabund[, 14:1034]
...
ndis <- n_distinct(taxabund[,c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus")])
reso <- 400
w_cutoff <- 0.01
kw_cutoff <- 0.01
l_cutoff <- 4
...
p_all <- ggpubr::ggarrange(p_1, p_3, nrow = 1, ncol = 2, 
                           labels = c("A) Spring 2021", "B) Summer 2021")) 
...
# lefse analysis
mm_lefse_0 <- run_lefse(divexp_phy_small, group = "Cropping.system", wilcoxon_cutoff = w_cutoff, kw_cutoff = kw_cutoff, lda_cutoff = 3)
...
ggpubr::ggexport(p_overall, # + labs(tag = "B) Fungi"), 
                 width = 3000, height = 2000, res = reso, 
...
mm_lefse_5@marker_table <- mm_lefse_5@marker_table[-c(4:8),]
gps <- unique(mm_lefse_5@marker_table$enrich_group)
...
mm_lefse_6@marker_table <- mm_lefse_6@marker_table[-c(3:7),]
gps <- unique(mm_lefse_6@marker_table$enrich_group)
...
#mm_lefse_12@marker_table <- mm_lefse_12@marker_table[-c(56:58),]

p_12 <- plot_cladogram(mm_lefse_12, color = cropsyscols[gps], 
                       alpha = 0.5, clade_label_level = 5, ) + 
  theme(plot.margin = margin(0, 0, 0, 0)) +   guides(fill = "none") 
ggpubr::ggexport(p_12, width = 6000, height = 3000, res = reso, 
                 filename = paste0("Figures/microbial-diversity/lefse/", region, "_Fall 2024_Cropping.system.png"))


### export plots for all timepoints

p_all <- ggpubr::ggarrange(NA, p_2, NA,
                           p_4, p_5, p_6, 
                           p_7, p_8, p_9, 
                           p_10, p_11, p_12, nrow = 4, ncol = 3, 
                           labels = c(NA, "A) Summer 2021", NA, 
                                      "B) Spring 2022", "C) Summer 2022", "D) Fall 2022", 
                                      "E) Spring 2023", "F) Summer 2023", "G) Fall 2023", 
                                      "H) Spring 2024", "I) Summer 2024", "J) Fall 2024"))
ggpubr::ggexport(p_all, width = 6000, height = 6500, res = reso, 
